...
import { QueryParametersDTO, PagedResult, SortedIncidentdsDTO } from '../models/Incidents';
...
@Component({
  ...
  templateUrl: './assignation.html',
  styleUrls: ['./assignation.css'],
})
export class Assignation implements OnInit {
  ...
  async ngOnInit() {
    ...
    async loadIncidents() {
      ...
      try {
        let result: PagedResult<SortedIncidentdsDTO> = await this.apiService.GetNotAssignedSortedIncidents(filter);
